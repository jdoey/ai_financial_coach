...
| **AI Engine**       | Google Gemini API (gemini-flash-latest) <br> Category-level Isolation Forest Model | Fast inference speeds suitable for real-time chat and structured JSON generation. <br> Suitable for the time constraints. Doesn’t require labeled datasets. Input raw data and get immediate results. |
